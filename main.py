...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Buy video
        ...
        # If user is admin, deliver video directly
        if is_admin:
            await query.message.reply_text("Admin access: Delivering video directly.")
            await payment_handler.deliver_video(update, context, video_id)
        else:
            # Otherwise, proceed with payment invoice
            await payment_handler.create_invoice(update, context, video_id)
    ...
    # Watch video
        ...
        if is_admin:
            await query.message.reply_text("Admin access: Delivering video directly.")
            await payment_handler.deliver_video(update, context, video_id)
        # Check if user has actually purchased this video (for non-admins)
        elif db.has_purchased(user_id, video_id):
            await payment_handler.deliver_video(update, context, video_id)
        else:
            await query.edit_message_text(
