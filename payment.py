...
class PaymentHandler:
    ...
    async def deliver_video(self, update: Update, context: ContextTypes.DEFAULT_TYPE, video_id: str) -> None:
        ...
        # Verify the user has purchased this video OR is an admin
        user_id = update.effective_user.id
        is_admin = self.db.is_admin(user_id) or user_id in config.ADMIN_USER_IDS

        # If user is not admin, check purchase status
        if not is_admin and not self.db.has_purchased(user_id, video_id):
